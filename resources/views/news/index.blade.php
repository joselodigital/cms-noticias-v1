<x-public-layout>
    <x-slot name="meta_title">Noticias</x-slot>
    <x-slot name="meta_description">Últimas noticias y actualizaciones de {{ config('app.name') }}.</x-slot>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div class="mb-8 border-b pb-4 border-gray-200/70 dark:border-gray-700/70">
            <h1 class="text-3xl md:text-4xl font-extrabold text-gray-900 dark:text-gray-50 tracking-tight">
                @if(request('search'))
                    Resultados de búsqueda: <span class="text-blue-600">"{{ request('search') }}"</span>
                @else
                    Noticias
                @endif
            </h1>
            <p class="text-gray-600 dark:text-gray-300 mt-3 text-sm md:text-base">
                @if(request('search'))
                    Se encontraron {{ $posts->total() }} resultados.
                @else
                    ...
                    <div class="bg-white/95 dark:bg-gray-900/90 backdrop-blur rounded-2xl shadow-md hover:shadow-xl hover:-translate-y-1 transition duration-300 flex flex-col h-full border border-gray-100 dark:border-gray-700">
                        ...
                        <div class="p-6 flex-grow flex flex-col">
                            <div class="text-xs text-gray-500 dark:text-gray-400 mb-2 flex justify-between">
                                ...
                                <a href="{{ route('news.author', $post->author) }}" class="font-medium text-gray-700 dark:text-gray-200 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors duration-200">{{ $post->author->name }}</a>
                            </div>
        ...
        @else
            <div class="text-center py-20 bg-white dark:bg-gray-900/80 rounded-lg shadow border border-gray-100 dark:border-gray-700">
                <h2 class="text-xl font-medium text-gray-600 dark:text-gray-200">No hay noticias disponibles.</h2>
            </div>
        @endif
    </div>
</x-public-layout>
