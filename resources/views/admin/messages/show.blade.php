<x-admin-layout>
    <div class="container mx-auto px-6 py-8">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-3xl font-bold text-gray-800 dark:text-white">Detalle del Mensaje</h2>
            <a href="{{ route('admin.messages.index') }}" class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">
                Volver
            </a>
        </div>

        <div class="bg-white dark:bg-slate-800 shadow-md rounded-lg overflow-hidden p-6 border border-gray-200 dark:border-slate-700">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Nombre</label>
                    <div class="text-lg text-gray-900 dark:text-white">{{ $message->name }}</div>
                </div>
                
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Fecha</label>
                    <div class="text-lg text-gray-900 dark:text-white">{{ $message->created_at->format('d/m/Y H:i') }}</div>
                </div>
                ...
                <div>
                    ...
                    <div class="text-lg text-gray-900 dark:text-white">
                        @if($message->whatsapp)
                            <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $message->whatsapp) }}" target="_blank" class="text-green-600 hover:underline flex items-center gap-1 dark:text-green-400">
                                {{ $message->whatsapp }}
                        ...
                        @else
                            <span class="text-gray-500 italic dark:text-slate-400">No proporcionado</span>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <div class="mb-6">
                <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">Mensaje</label>
                <div class="p-4 bg-gray-50 dark:bg-slate-700 rounded-lg text-gray-900 dark:text-white whitespace-pre-wrap">{{ $message->message }}</div>
            </div>
